...
begin
   if not (MastData.OrdersSaleDate.AsString = '') then
...
procedure TEdOrderForm.CloseBtnClick(Sender: TObject);
begin
Close;
end;

procedure TEdOrderForm.SpeedButton9Click(Sender: TObject);
begin
RPoint:=CustNoEdit.Text;
end;

procedure TEdOrderForm.SpeedButton8Click(Sender: TObject);
begin
 if not MastData.Orders.locate('CustNo',RPoint,[])then
    ShowMessage('Sorry.... Maybe You didnt make return point');
end;

procedure TEdOrderForm.SpeedButton1Click(Sender: TObject);
begin
RPoint2:=CustNoEdit.Text;
end;

procedure TEdOrderForm.SpeedButton2Click(Sender: TObject);
begin
 if not MastData.Orders.locate('CustNo',RPoint2,[])then
    ShowMessage('Sorry.... Maybe You didnt make return point');
end;

procedure TEdOrderForm.ActiveSourceDataChange(Sender: TObject;
  Field: TField);
begin
 PostBtn.Enabled := MastData.Orders.State in dsEditModes;
  CancelBtn.Enabled := PostBtn.Enabled;
